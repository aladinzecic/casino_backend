...
router.get('/user', async (req, res) => {
    ...
    try {
        ...
        const [rows] = await db.query('SELECT  username, razred,caracterNum,caracterEvolLevel,dayStreak,totalXp,numOfCookies,dailySpinsLeft,mathCurr,englishCurr,infoCurr,geoCurr FROM user WHERE id = ?', [userId]);
...
router.get('/getPage', async (req, res) => {
    ...
    try {
        ...
        // Upit za pronalaženje korisnika na osnovu userId
        const [rows] = await db.query('SELECT  rouletteVisited, plinkoVisited,huntVisited,minesVisited FROM user WHERE id = ?', [userId]);
...
router.post('/updatePage', async (req, res) => {
    ...
    try {
      ...
      if(page==="ROULLETE"){
        await db.query(
          'UPDATE user SET rouletteVisited = ? WHERE id = ?',
          [true, userId]
        );
      }
      else if(page==="PLINKO"){
        await db.query(
          'UPDATE user SET plinkoVisited = ? WHERE id = ?',
          [true, userId]
        );
      }
      else if(page==="HUNT"){
        await db.query(
          'UPDATE user SET huntVisited = ? WHERE id = ?',
          [true, userId]
        );
      }
      else if(page==="MINES"){
        await db.query(
          'UPDATE user SET minesVisited = ? WHERE id = ?',
          [true, userId]
        );
      }
  ...
  router.get('/getMoney/:userId', async (req, res) => {
    ...
    try {
      ...
      console.error(err);
      res.status(500).json({ message: 'Error fetching user money' });
    }
  });
  ...
  try {
    ...
    const newUsers = await db.query(
      'SELECT COUNT(*) AS dailyUsers FROM user WHERE DATE(FROM_UNIXTIME(createdAt / 1000)) = CURDATE()'
    );
    
    const activeUsers=await db.query(
      'SELECT COUNT(*) AS activeUsers FROM user WHERE lastActiveAt > UNIX_TIMESTAMP(DATE_SUB(NOW(), INTERVAL 3 MINUTE)) * 1000;'
    )
...
1    )
    ...
    // Greška na serveru
    res.status(500).json({ message: 'Error fetching user count', error: e.message });
  }
});
...
router.post('/updateActivity', async (req, res) => {
  ...
  try {
    const [result] = await db.query(
      'UPDATE user SET lastActiveAt = ? WHERE id = ?',
      [Date.now(), userId]
    );
...
router.post('/deposit', async (req, res) => {
  ...
  try {
    const [result] = await db.query(
      'UPDATE user SET depositedMoney = ? WHERE id = ?',
